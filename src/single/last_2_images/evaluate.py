...
def evaluate(seed, device_idx=0):
    ...
    for fold in GeneralCFG.train_fold:
        input_dir = Last2ImagesCFG.output_dir / f"seed{seed}"
        model = LitModel()
    ...
    oof_df.to_csv(Last2ImagesCFG.output_dir / "oof.csv", index=False)
...
if __name__ == "__main__":
    Last2ImagesCFG.output_dir = Path("/workspace", "output", "single", "last_2_images", "resnet50")
