...
if __name__ == '__main__':
    ...
    large_checkpoint = torch.load("pretrained_weights/nextvit_large_in1k_384.pth")
    model = timm.create_model("nextvit_large", pretrained=False, num_classes=1000, )
    model.load_state_dict(large_checkpoint['model'])
    # model の、stemの最初のConvBNReLUの、入力チャンネルを1に変更する
    model.stem[0].conv.in_channels = 1
    # model の、stemの最初のConvBNReLUの、重みを1チャンネルのものに変更する
    model.stem[0].conv.weight.data = model.stem[0].conv.weight.data[:, 0:1, :, :]

    # model を特徴抽出器として利用するため、最後の分類層を削除する
    model = nn.Sequential(*list(model.children())[:-1])

    print(torchinfo.summary(model, input_size=(1, 1, 1536, 1410), verbose=2, device="cpu"))
    output = model(torch.randn(1, 1, 1536, 1410))
