...
class ValidNetMixin(pl.LightningModule):
    ...
    def validation_epoch_end(self, validation_step_outputs) -> None:
        ...
        valid_df = load_processed_data_pol.sample_oof(seed=seed_num).filter(
        ...
        all_labels = valid_df.select(pol.col("cancer")).to_numpy().flatten()
        all_preds = valid_df.select(pol.col("all_preds")).to_numpy().flatten()
